...
public class FavouriteController {
    ...
    @ApiOperation("Return Favourites by User id")
    @GetMapping("/users/{userId}/favourites")
    public ResponseEntity<List<FavouriteResource>> getAllFavouritesByUserId(@PathVariable(name = "userId") Integer userId)
    {
        User existingUser = userService.getUserById(userId);
        if(existingUser == null)
            return ResponseEntity.notFound().build();
        var favourites = existingUser.getFavourites();
